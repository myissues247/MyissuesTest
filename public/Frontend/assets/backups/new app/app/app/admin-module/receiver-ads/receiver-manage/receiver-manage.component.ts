...
export class ReceiverManageComponent implements OnInit {
  ...
  getReceivers(withTotal) {
    this.loading = true;
    this.ds.GetUser({withTotal, skip: this.skip, limit: this.limit}).subscribe((response)=>{
      this.loading = false;
      console.log(response);
      if(response.status==true){
          this.post=response.data.receivers;   
          this.totalReceivers = response.data.total || this.totalReceivers;
      } else {
  ...
  loadCustomers(event:LazyLoadEvent){
    ...
    this.getReceivers(!this.totalReceivers);
    // setTimeout(() => {
    //   this.loading = true;
    //   this.ds.GetUser({withTotal:!this.totalReceivers, skip: event.first, limit: event.rows }).subscribe((response)=>{
    //     console.log(response);
    //     this.loading = false;
    //     if(response.status==true){
    //         this.post=response.data.receivers;   
