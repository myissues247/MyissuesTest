...
export class TransactionComponent implements OnInit {
 ...
 async ngOnInit() {
  ...
  getTransaction() {
    this.loading = true;
    this.ds.get_transactions_all({userId: this.id}).subscribe((response)=>{
      this.loading = false;
  ...
  loadTransactions(event:LazyLoadEvent){
    ...
    //   this.ds.get_transactions_all({userId: this.id,skip: event.first, limit: event.rows}).subscribe((response)=>{
    //     this.loading = false;
    //     if(response.status==true){
    //         this.post=response.data;
    //     } else {
