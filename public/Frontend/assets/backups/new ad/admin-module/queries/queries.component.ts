...
export class QueriesComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute,private ds:AdminDataService,private router:Router,private cnfrm: ConfirmationService, private messageService: MessageService) {
    this.items = [
      ...
      { icon: 'pi pi-pw pi-comments', label: ' Quereies' }
    ];
   }
  ...
  loadQueries(event:LazyLoadEvent){
    setTimeout(() => {
      this.loading = true;

      this.ds.get_queries({withTotal:true, skip: event.first, limit: event.rows}).subscribe((response)=>{
        this.loading = false;
        if(response.status==true){
