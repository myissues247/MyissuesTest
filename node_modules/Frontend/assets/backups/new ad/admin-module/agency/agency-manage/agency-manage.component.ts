...
export class AgencyManageComponent implements OnInit {
  ...
  getAgencies(withTotal) {
    this.loading = true;
    this.ds.getAllAgencyDetail({withTotal}).subscribe((response) => {
      this.loading = false;
      if (response.status) {
        this.totalAgencies = response.data.total;
      }
      else
 ...
 loadAgency(event:LazyLoadEvent) {
  setTimeout(() => {
    this.loading = true;
    this.ds.getAllAgencyDetail({withTotal:true,skip: event.first, limit: event.rows}).subscribe((response) => {
      this.loading = false;
      if (response.status) {
