...
export class UserDetailsComponent implements OnInit, OnDestroy, AfterViewInit {  
  ...
  saved: boolean = false;
  constructor(private ps : SponsorAdService, private ds:UserDataService, private router: Router, private ss: StateService, private loader: LoaderService) { 
  ...
  nextPage(form){
    //TODO : implement changes and unsaved data detection.
     ...
     this.save_contact().then(()=>{ 
       this.stopload();
      setTimeout(()=>{
        this.router.navigate(['/sponsor/post/upgrades'], {queryParams: {mode: "preview"}});
       }, 1000);
     }).catch(()=>this.stopload())
  }
