...
export class FullComponent implements OnInit {
  ...
  constructor(private ds: UserDataService, private ar: ActivatedRoute, private router: Router, private ps: AdPostService, private ss: StateService, private ren: Renderer2) {
    ...
    this.adData = {
      ...
      age : 0,
      city : "Loading....",
      identityImages : ["assets/images/banner2.jpg"],
  ...
  setThumbnail() {
    if(this.adData.identityImages instanceof Array && this.adData.identityImages.length > 0)
      this.thumbnail = environment.server + '/' + this.adData.identityImages[0];
    else if(this.adData.adImages instanceof Array && this.adData.adImages.length > 0)
      this.thumbnail = environment.server + '/' + this.adData.adImages[0];
    else 
      this.thumbnail =  'assets/images/no-image.jpg';
  }
  ...
  initShare() {
    ...
    const image = encodeURIComponent((this.adData.adImages?.length> 0)? (environment.server + '/' + this.adData.adImages[0]): environment.server + '/assets/frontend/no-image.jpg' )
  ...
  handle_favorite(event, removeConfirm) {
    ...
    const data : any= {
      adId: this.adId,
      ownerType: this.adData.ownerType, 
      receiverId: this.adData.receiverId, 
      age: this.adData.age, 
      adTitle: this.adData.adTitle,
      city: this.adData.city, fullname,
      approvedOn: this.adData.approvedOn
    }

    if(this.adData.identityImages?.length > 0)
      data.image = this.adData.identityImages[0];
    else 
      data.image = this.adData.adImages[0];

    this.ds.make_ad_favorite(data).subscribe((response)=>{
      if(response.status) {
