...
export class EditAdComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.ds.getSponsorSingleAd(this.id).subscribe(async (response) => {
      if (response.status) {
...
change_state(event): Promise<boolean>{
  ...
  deleteAdImage(adImage, i) {
    this.cnfrm.confirm({
      ...
      accept: () => {
          this.loading = true;
          this.ds.deleteAdImage_sponsor(this.siteDetail[0]._id, adImage).subscribe((response)=>{
            this.loading = false;
            if(response.status) {
  ...
  update_ad(editForm) {
    ...
    if(editForm.status == "INVALID" || !this.siteDetail[0].description || !this.siteDetail[0].city || !this.siteDetail[0].state || !this.siteDetail[0].country || /* !this.siteDetail[0].categoryName || */ !this.siteDetail[0].contact)
      ...
      this.cnfrm.confirm({
        ...
        icon: 'pi pi-exclamation-triangle',
        accept: () => {
          this.ds.updateAd_sponsor(this.siteDetail[0]).subscribe((response)=>{
            if(response.status) {
              this.messageService.add({severity: 'success', summary: 'Ad updated', detail: response.message});
              setTimeout(()=>{this.router.navigate(["/admin-module/sponsor/ad/", this.siteDetail[0]._id])}, 1000);
            } else {
