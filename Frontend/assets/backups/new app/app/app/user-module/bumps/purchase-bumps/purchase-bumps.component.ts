...
export class PurchaseBumpsComponent implements OnInit, OnDestroy {
  ...
  constructor(private ps : UserDataService, private router : Router, private ss: StateService) { }
  ...
  getAdPlan(){
    ...
    this.ps.get_bumps().pipe(takeUntil(this.unsub$)).subscribe((response)=>{
      if(response.status){
  ...
  buyBumps() {
    ...
    this.router.navigate(['/receiver/bumps/pay-with-card'], {queryParams: {bumpId: bump._id, price: bump.price, count: bump.count}})
  }
