...
import { Ad, AgencyAd } from 'src/app/services/dataModels';
...
export class FinalizeComponent implements OnInit {
  ...
  plan : any;
  session;
  constructor(private router:Router, private ar: ActivatedRoute, private ps: AgencyAdServiceService, private loader: LoaderService, private ss: StateService) { }
  ...
  ngOnInit(): void {
    ...
    this.ps.AdObservable().subscribe((ad)=>{
      ...
      if(!this.ad._id) {
        ...
        this.router.navigate(['/agency/post/select-category']);
      }
  ...
  publish() {
      ...
      this.load();
    this.ps.publish_normal(this.ad._id).subscribe((response)=>{
      this.stopload();
      if(response.status) {
  ...
  handle_upgrade() {
    ...
    this.router.navigate(['/agency/post/pay-via-card'], {queryParams: {action: 'upgrade'}});
  }


  prevPage(){
    /* TODO : implement local cache for unsaved data; */
    this.router.navigate(['/agency/post/upgrades']);
  }
