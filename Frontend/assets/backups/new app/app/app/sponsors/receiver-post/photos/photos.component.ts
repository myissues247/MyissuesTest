...
export class PhotosComponent implements OnInit {
  ...
  constructor(private ps:SponsorAdService, private router: Router, private loader: LoaderService) {
     ...
     this.ps.save_ad_images(files).subscribe((event)=>{
      if (event.type == HttpEventType.UploadProgress) {
        ...
        if(response.status){
          ...
          adImageUpload.clear();
          adImageUpload.uploadedFileCount = 0 // resets the uploaded count so choose button doesn't get disabled
          /****** NOTE : PLEASE CHANGE ITS IMPLEMENTATION TO ONLY SAVE CHANGED FIELDS ***/
