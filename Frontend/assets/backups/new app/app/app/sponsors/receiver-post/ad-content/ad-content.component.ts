...
export class AdContentComponent implements OnInit, AfterViewInit {
  ...
  saved : boolean = false;
  constructor(private ren : Renderer2, private ps: SponsorAdService, private router: Router, private loader: LoaderService, private ss: StateService) { 
    this.adContent = {
      adTitle : "",
      sponsorTitle: "",
      description : "",
  ...
  ngOnInit(): void {
    ...
    this.ps.AdObservable().subscribe((ad: Ad)=>{
      ...
      this.adContent.sponsorTitle = ad.sponsorTitle? ad.sponsorTitle : "";
      ...
      if(ad.adTitle && ad.description && ad.sponsorTitle)
        this.saved = true;
    });

    this.ss.getUserSessionObservable().subscribe((session)=> this.adContent.sponsorTitle = session.sponsorTitle);
  }
 ...
 nextPage(form){
    ...
    if(this.saved){
      ...
      if(this.checkUnsavedChanges()){
        this.handleUnsavedChanges().then(()=>{
          this.stopload();
          this.router.navigate(['/sponsor/post/upload-images']);
        }).catch(()=>this.stopload());
      } else {
        this.stopload();
        this.router.navigate(['/sponsor/post/upload-images']);
  ...
  checkUnsavedChanges() : boolean{
    ...
    if(this.ad.sponsorTitle != this.adContent.sponsorTitle || this.ad.description != this.adContent.description || this.ad.adTitle != this.adContent.adTitle || this.ad.tagline != this.adContent.tagline)
      return true;
    return false; 
  }
