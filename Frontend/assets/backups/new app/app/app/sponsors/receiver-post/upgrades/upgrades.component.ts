...
import { SponsorDataService } from 'src/app/sponsors/services/sponsor-data.service';
...
export class UpgradesComponent implements OnInit, OnDestroy {
  ...
  constructor(private router:Router, private ps: SponsorAdService, private sds: SponsorDataService, private ar: ActivatedRoute, private loader: LoaderService, private ss: StateService) { 
  ...
  checkParamForAdId() {
      ...
      this.ar.queryParams.pipe(takeUntil(this.unsub$)).subscribe((params)=>{ 
        ...
        this.router.navigate(['/sponsor/post/select-location']);
       });
  }
  ...
  buyBumps() {
    ...
    this.router.navigate(['/sponsor/bumps/pay-with-card'], {queryParams: {bumpId: bump._id, price: bump.price, count: bump.count, back: '/sponsor/post/upgrades', adId: this.ad._id}});
  }
  ...
  nextPage() {
    /* TODO : implement changes and unsaved data detection. */
     this.save_plan().then(()=>{ 
       this.router.navigate(['/sponsor/post/finalize']);
     });
  }

  prevPage() {
    //TODO : implement local cache for unsaved data;
    this.router.navigate(['/sponsor/post/user-details']);
  }
  ...
  buyBumpWithBalance() {
    ...
    this.sds.purchase_bump_up_with_bal({bumpId: this.selectedBump._id}).pipe(takeUntil(this.unsub$)).subscribe((response)=>{
       if(response.status) {
