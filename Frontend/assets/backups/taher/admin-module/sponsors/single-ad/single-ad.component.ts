...
export class SingleAdComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    await this.ds.getSponsorSingleAd(this.id).subscribe(async (response) => {
      if (response.status) {
      ...
      accept: () => {
        this.ds.approveSponsorAd({ adId: adId }).subscribe((response) => {
          if (response.status == true)
          {
            ...
            this.siteDetail[0].approved = true;
          }
          else
  ...
  disableAd(){
    this.ds.rejectSponsorAd({ adId: this.adId,rejectionCauses:this.cause }).subscribe((response) => {
      if (response.status == true)
      {
        ...
        this.siteDetail[0].approved = false;
        this.cause=null;
      }
      else
        alert(response.message)
  ...
  disable(ev){
    this.cnfrm.confirm({
      ...
      icon: 'pi pi-exclamation-triangle',
      accept: () => {
        this.ds.updateSponsorAd({ adId: ev, status: false }).subscribe((response) => {
          if (response.status == true)
          {
            this.messageService.add({ severity: 'warn', summary: 'Deactivated', detail: response.message });
