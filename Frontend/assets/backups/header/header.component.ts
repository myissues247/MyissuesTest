...
export class HeaderComponent implements OnInit, AfterViewInit {
 ...
 resendVerificationLink() {
  ...
  if(!this.pwdMailProp.match(pattern))
    ...
    this.ds.resendVerification({email: this.pwdMailProp, type: this.roleProp.code}).subscribe((response)=>{
      if(response.status) {
  ...
  setPwd(){
   ...
   return document.getElementById('pwdErr').textContent='Please Enter valid Mail Id';
     ...
     console.log(this.roleProp.code);
      this.ds.resetPassword({email:this.pwdMailProp, type: this.roleProp.code}).subscribe(async (response)=>{
        if(response.status) {
