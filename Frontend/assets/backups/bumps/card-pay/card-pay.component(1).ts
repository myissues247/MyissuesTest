...
export class CardPayComponent implements OnInit {
  ...
  states= [];
  cities = [];
  constructor(private cd: ChangeDetectorRef, private uds: UserDataService, private ar: ActivatedRoute, private ds: AgencyDataService, private router: Router, private ss: StateService) {
    ...
    this.ar.queryParams.pipe(takeUntil(this.unsubscribe$)).subscribe((data)=>{
      if(data.bumpId) {
        ...
        if(data.back) {
          ...
          if(data.back == '/agency/post/upgrades') {
             if(data.adId)
               this.back.params.adId = data.adId;
          }
        } else {
            this.back.link = '/agency';
        }
      }
    });
  }
  ...
  completeOrder(data) {
    ...
    this.ds.purchase_bump_up(data).pipe(takeUntil(this.unsubscribe$)).subscribe((response)=>{
      ...
      if(response.status) {
        ...
        this.router.navigate(['/agency/payment-status'], {queryParams: {type: 'purchase', success: true, paymentIntentId: response.data.paymentIntent, transactionId: response.data.transactionId, backLink: this.back.link, adId: this.back.params?.adId, amount: this.bumpUpData.price, bumps: this.bumpUpData.count, date: new Date()}});
      } else {
