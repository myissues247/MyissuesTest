...
export class EditAdComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private ds: AdminDataService, private cnfrm: ConfirmationService, private messageService: MessageService, private router: Router) {
    this.items = [
      ...
      { icon: 'pi pi-pw pi-pencil', label: ' Edit Ads Detail' }
    ]
  }

  async ngOnInit() {
    this.id = await this.route.snapshot.paramMap.get('id');
    this.ds.getSingleAdData(this.id).subscribe(async (response) => {
      if (response.status) {
        console.log(response.data);
        this.siteDetail = [response.data];
        console.log(this.siteDetail);
        this.get_ad_options();
        const currentDob = new Date(this.siteDetail[0].dob);
        this.dobConverted = currentDob.getFullYear() + '-' + (currentDob.getMonth() + 1) + '-' + currentDob.getDate();
      }
    }, (err) => {
      console.log(err)
    });
  ...
  get_ad_options() : any {
    this.loading = true;
    this.ds.get_edit_ad_selector().subscribe((response:any)=>{
      this.loading = false;
      if(response.status){
...
change_state(event): Promise<boolean>{
  ...
  deleteAdImage(adImage, i) {
    this.cnfrm.confirm({
      ...
      accept: () => {
          this.loading = true;
          this.ds.deleteAdImage(this.siteDetail[0]._id, adImage).subscribe((response)=>{
            this.loading = false;
            if(response.status) {
  ...
  update_ad(editForm) {
    ...
    this.siteDetail[0].dob = this.dobConverted;
    this.ds.updateAd(this.siteDetail[0]).subscribe((response)=>{
      if(response.status) {
        this.messageService.add({severity: 'success', summary: 'Ad updated', detail: response.message});
        setTimeout(()=>{this.router.navigate(["/admin-module/receiver-ads/full/", this.siteDetail[0]._id])}, 1000);
      } else {
