...
export class LocationComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.get_country().then(()=>{
      ...
      const unsubscribe$ = this.ps.AdObservable().subscribe((ad: Ad)=>{
        if(ad.country){
  ...
  change_state(event): Promise<boolean>{
     ...
     this.loading = true;
     return new Promise((res, rej)=>{
        this.ds.get_cities(this.selectedCountry, this.selectedState).subscribe((response:any)=>{
          this.loading = false;
          this.showCity = true;
          if(response.status){
  ...
  save_location(){
    if(this.emptySelect())
      return;
    this.load();
    this.ps.selectLocation(this.selectedCountryName, this.selectedState, this.selectedCity).subscribe((response: any)=>{
      if(response.status){
